    ...
    <TestRun xmlns="http://microsoft.com/schemas/VisualStudio/TeamTest/2010">
      ...
      <Times>
        <xsl:attribute name="creation">
          <xsl:value-of select="concat(/test-results/@date,'T',/test-results/@time,'-06:00')"/>
        </xsl:attribute>
      ...
      <TestDefinitions>
        <xsl:for-each select="//test-case">
          ...
          <UnitTest>
            ...
            <TestMethod adapterTypeName="xUnit" >
              <xsl:attribute name="name">
                <xsl:value-of select="@name"/>
              </xsl:attribute>
              ...
              <xsl:attribute name="className">
                <xsl:variable name="testClassName">
                  <xsl:call-template name="getTestClassName">
                    <xsl:with-param name="nunitName" select="NotHere"/>
                  </xsl:call-template>
                </xsl:variable>
                <xsl:value-of select="concat('NA', ', ', $testClassName)" />
              </xsl:attribute>
            </TestMethod>
          </UnitTest>
        </xsl:for-each>
